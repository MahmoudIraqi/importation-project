...
} from '../comments/view-release-comments/view-release-comments.component';
...
export class CustomReleaseComponent implements OnInit, OnDestroy {
  ...
  SubmitErrorValidationHandling() {
    if (this.regCustomReleaseForm.value.invoices.length === 0) {
      this.handleError('Please enter at least 1 invoice on the request');
    }
    if (this.regCustomReleaseForm.value.invoices.length === 0) {
      this.handleError('Please enter at least 1 invoice on the request');
    }
